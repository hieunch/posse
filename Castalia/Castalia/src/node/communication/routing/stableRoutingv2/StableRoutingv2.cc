#include "StableRoutingv2.h"
#include <crng.h>
#include "assert.h"

Define_Module(StableRoutingv2);

int StableRoutingv2::cnt = 0;
int StableRoutingv2::nextId;
map<tuple<tuple<Point, Point>, Point, Point, int>, pair<vector<Point>, int> > StableRoutingv2::outCavernCache;
map<tuple<Point, Point, double>, vector<Point> > StableRoutingv2::aroundHoleCache;
map<tuple<Point, Point, int, double, int>, vector<Point> > StableRoutingv2::findPathCache;

void StableRoutingv2::startup(){
...
void StableRoutingv2::processBufferedPacket(){
...
void StableRoutingv2::fromApplicationLayer(cPacket * pkt, const char *destination){
...
void StableRoutingv2::fromMacLayer(cPacket * pkt, int macAddress, double rssi, double lqi){
...
void StableRoutingv2::finishSpecific() {
...
void StableRoutingv2::processDiscoverHolePacket(DiscoverHolePacket* pkt){
...
void StableRoutingv2::propagateHole(DiscoverHolePacket *pkt) {
...
// the node's position
void StableRoutingv2::handleNetworkControlCommand(cMessage *msg) {

}

vector<Point> StableRoutingv2::findPath(Point from, Point to, vector<Point> &hole, vector<vector<CavernNode>> &caverns, int id) {
  vector<Point> result;
...
pair<vector<Point>, int> StableRoutingv2::findPathOutCavern2(Point from, Point to, vector<Point> &hole,
  ...
  if (!G::isPointInsidePolygon(to, convexHull)) {
    ...
    vector<Point> d_sp1;
    for (int i=0; i<d_sp1Index.size()-1; i++) d_sp1.push_back(cavern[d_sp1Index[i]]);
    d_sp1.push_back(to);
    vector<Point> d_sp2;
    for (int i=0; i<d_sp2Index.size()-1; i++) d_sp2.push_back(cavern[d_sp2Index[i]]);
    d_sp2.push_back(to);
...
// vector<Point> StableRoutingv2::findPathAroundHole(Point from, Point to, vector<Point> &hole, double r) {
...
vector<Point> StableRoutingv2::bypassHole(double sDist1, double sDist2, double dDist1, double dDist2, int s1, int s2, int d1, int d2, vector<Point> p) {
...
vector<int> StableRoutingv2::bypassHole2(double sDist1, double sDist2, double dDist1, double dDist2, int s1, int s2, int d1, int d2, vector<Point> p) {
    Point ms = G::midpoint(p[s1], p[s2]);
    Point md = G::midpoint(p[d1], p[d2]);
...
void StableRoutingv2::findViewLimitVertices(Point point, std::vector<Point> polygon, int &i1, int &i2) {
...
double StableRoutingv2::calculatePathLength(vector<Point> p) {
    double length = 0;
...
double StableRoutingv2::computeLevel(vector<Point> hole, int i, double max){
  int n = hole.size();
  Point p = hole[i];
