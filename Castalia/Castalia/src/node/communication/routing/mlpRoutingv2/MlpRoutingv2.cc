#include "MlpRoutingv2.h"
#include <crng.h>
#include "assert.h"

Define_Module(MlpRoutingv2);

int MlpRoutingv2::cnt = 0;
int MlpRoutingv2::nextId;
double MlpRoutingv2::eps;
map<tuple<tuple<Point, Point>, Point, Point, int>, tuple<vector<Point>, int, double> > MlpRoutingv2::outCavernCache;
map<tuple<Point, Point, double>, vector<Point> > MlpRoutingv2::aroundHoleCache;
map<tuple<Point, Point, int, double, int>, vector<Point> > MlpRoutingv2::findPathCache;

void MlpRoutingv2::startup(){
  eps = par("eps");
...
void MlpRoutingv2::processBufferedPacket(){
...
void MlpRoutingv2::fromApplicationLayer(cPacket * pkt, const char *destination){
...
void MlpRoutingv2::processDiscoverHolePacket(DiscoverHolePacket* pkt){
...
void MlpRoutingv2::propagateHole(DiscoverHolePacket *pkt) {
...
void MlpRoutingv2::processDataPacket(MlpPacket* pkt){
  ...
  if (pkt->getNextStoppingPlace().isUnspecified()) {
    ...
    if (receivedHole) {
      ...
      pkt->setPreviousStoppingPlace(selfLocation);
    ...
    Point previousStoppingPlace = pkt->getPreviousStoppingPlace();
    if (((nextStoppingPlace != destLocation) && GlobalLocationService::isInSameCell(selfLocation, nextStoppingPlace)) || (selfLocation == destLocation)) {
      ...
      if (pkt->getRoutingMode() == MLP_GREEDY_ROUTING) {
        ...
        for (auto &neighbor: neighborTable) {
          ...
          //     break;
          // }
          // if (!G::is_intersect(XY, GlobalLocationService::getCell(neighbor.location))) continue;
...
// the node's position
void MlpRoutingv2::handleNetworkControlCommand(cMessage *msg) {

}

vector<Point> MlpRoutingv2::findPath(Point from, Point to, vector<Point> &hole, vector<vector<CavernNode>> &caverns, int id) {
  vector<Point> result;
  ...
  for (auto cavern: caverns) {
    if (G::pointInOrOnPolygon(cavern, from)) {
      ...
      outCavern = get<0>(outCavernPair);
      side_out = get<1>(outCavernPair);
      translateFactor = get<2>(outCavernPair);
      trace() << "side_out " << side_out;
  ...
  for (auto cavern: caverns) {
    if (G::pointInOrOnPolygon(cavern, to)) {
      auto outCavernPair = findPathOutCavern2(to, from, hole, cavern, path);
      inCavern = get<0>(outCavernPair);
      if (translateFactor == -1) translateFactor = get<2>(outCavernPair);
  ...
  vector<Point> middlePath = findPathAroundHole(newFrom, newTo, hole, translateFactor);
  //for (int i=0; i<5; i++) trace() << "OK4";
  ...
  if (outCavern.empty()){
    ...
    if (middlePath.size()>1) result.insert(result.end(), middlePath.begin(), middlePath.end());
  }
  if (!inCavern.empty()){
    for (int i=1; i<inCavern.size(); i++){
    }
    result.insert(result.end(), inCavern.begin()+1, inCavern.end());
  }
  trace() << "shifted path length " << calculatePathLength(result);
  for (Point p : result) debugPoint(p, "red");

  auto flattenResult = G::flatten(result);
  debugPath(flattenResult, "red");

  return flattenResult;
}

tuple<vector<Point>, int, double> MlpRoutingv2::findPathOutCavern2(Point from, Point to, vector<Point> &hole,
  ...
  vector<Point> shortestPathEntire;
  int nHole = hole.size();
  ...
  if (!G::isPointInsidePolygon(to, convexHull)) {
    ...
    if (sp1.back() == sp_.front()) {
        ...
        shortestPathEntire = sp2;
    }
    shortestPathEntire.insert(shortestPathEntire.end(), sp_.begin() + 1, sp_.end());
    shortestPathEntire.push_back(to);
  }
  else {
    vector<Point> cavernPoints;
    for (Point p : cavern) cavernPoints.push_back(p);
    ...
    vector<Point> cavernPoints2;
    for (Point p : cavern2) cavernPoints2.push_back(p);
    ...
    DGraph *graph1 = new DGraph(cavernPoints);
    vector<int> s_sp1Index = graph1->shortestPath2(from, cavernPoints[0]);
    delete(graph1);
    DGraph *graph2 = new DGraph(cavernPoints);
    vector<int> s_sp2Index = graph2->shortestPath2(from, cavernPoints[cavernPoints.size() - 1]);
    delete(graph2);
    DGraph *graph3 = new DGraph(cavernPoints2);
    vector<int> d_sp1Index = graph3->shortestPath2(cavernPoints2[cavernPoints2.size() - 1], to);
    delete(graph3);
    DGraph *graph4 = new DGraph(cavernPoints2);
    vector<int> d_sp2Index = graph4->shortestPath2(cavernPoints2[0], to);
    delete(graph4);

    // DGraph *graph5 = new DGraph(cavernPoints);
    // vector<Point> s_sp1 = graph5->shortestPath(from, cavernPoints[0]);
    // delete(graph5);
    // DGraph *graph6 = new DGraph(cavernPoints);
    // vector<Point> s_sp2 = graph6->shortestPath(from, cavernPoints[cavernPoints.size() - 1]);
    // delete(graph6);
    // DGraph *graph7 = new DGraph(cavernPoints2);
    // vector<Point> d_sp1 = graph7->shortestPath(cavernPoints2[cavernPoints2.size() - 1], to);
    // delete(graph7);
    // DGraph *graph8 = new DGraph(cavernPoints2);
    // vector<Point> d_sp2 = graph8->shortestPath(cavernPoints2[0], to);
    // delete(graph8);

    vector<Point> s_sp1;
    s_sp1.push_back(from);
    for (int i=1; i<s_sp1Index.size(); i++) s_sp1.push_back(cavern[s_sp1Index[i]]);
    vector<Point> s_sp2;
    s_sp2.push_back(from);
    for (int i=1; i<s_sp2Index.size(); i++) s_sp2.push_back(cavern[s_sp2Index[i]]);
    vector<Point> d_sp1;
    for (int i=0; i<d_sp1Index.size()-1; i++) d_sp1.push_back(cavern2[d_sp1Index[i]]);
    d_sp1.push_back(to);
    vector<Point> d_sp2;
    for (int i=0; i<d_sp2Index.size()-1; i++) d_sp2.push_back(cavern2[d_sp2Index[i]]);
    d_sp2.push_back(to);
    ...
    vector<Point> sp_ = bypassHole(calculatePathLength(s_sp1),
        ...
        G::getVertexIndexInPolygon(cavernPoints[0], convexHull),
        G::getVertexIndexInPolygon(cavernPoints[cavernPoints.size() - 1], convexHull),
        G::getVertexIndexInPolygon(cavernPoints2[cavernPoints2.size() - 1], convexHull),
        G::getVertexIndexInPolygon(cavernPoints2[0], convexHull), convexHull);

    if (s_sp1.back() == sp_.front()) {
        // for (int i : s_sp1Index) shortestPath.push_back((i+startCavern)%nHole);
        for (int i=1; i<s_sp1Index.size(); i++) shortestPath.push_back(s_sp1Index[i]);
        shortestPathEntire = s_sp1;
        shortestPathEntire.insert(shortestPathEntire.end(), sp_.begin() + 1, sp_.end());
        if (sp_.back() == d_sp1.front()) {
            shortestPathEntire.insert(shortestPathEntire.end(), d_sp1.begin() + 1, d_sp1.end());
        } else if (sp_.back() == d_sp2.front()) {
            shortestPathEntire.insert(shortestPathEntire.end(), d_sp2.begin() + 1, d_sp2.end());
        }
    } else if (s_sp2.back() == sp_.front()) {
        // for (int i : s_sp2Index) shortestPath.push_back((i+startCavern)%nHole);
        for (int i=1; i<s_sp2Index.size(); i++) shortestPath.push_back(s_sp2Index[i]);
        shortestPathEntire = s_sp2;
        shortestPathEntire.insert(shortestPathEntire.end(), sp_.begin() + 1, sp_.end());
        if (sp_.back() == d_sp1.front()) {
            shortestPathEntire.insert(shortestPathEntire.end(), d_sp1.begin() + 1, d_sp1.end());
        } else if (sp_.back() == d_sp2.front()) {
  ...
  debugPath(shortestPathEntire, "black");
  double l_out = calculatePathLength(shortestPathEntire);
  // l_out += G::distance(from, shortestPathPoint[0]);
  ...
  Angle theta = 0;
  // for (int i = 1; i < L_out.size()-1; i++){
  //   Point X_i = cavern[L_out[i]];
  //   Point X_i1 = cavern[L_out[i-1]];
  //   Point X_i2 = cavern[L_out[i+1]];
  //   theta += M_PI - G::angle(&X_i, &X_i1, &X_i2);
  // }
  for (int i = 2; i < shortestPathEntire.size()-2; i++){
    Point X_i = shortestPathEntire[i];
    Point X_i1 = shortestPathEntire[i-1];
    Point X_i2 = shortestPathEntire[i+1];
    theta += M_PI - G::angle(&X_i, &X_i1, &X_i2);
  }
  ...
  // double maxLevel = 0;
  // for (int k : L_out){
  //   if (cavern[k].level_ > maxLevel) maxLevel = cavern[k].level_;
  // }
  // if (maxLevel > baseK){
    // for (int i=0; i<L_out.size()-1; i++){
    //   if (cavern[L_out[i]].level_ < baseK){
    //     result.push_back(cavern[L_out[i]]);
    //   }
    //   else {
    //     startShift = i;
    //     break;
  ...
  // }
  // for (int i : L_out){
  //   if (cavern[i].level_ < baseK) baseK = cavern[i].level_;
  // }
  ...
  int side0 = 1;
  double translateFactor = -1;
  // if (L_out.size() >= 2) side0 = G::isMonotoneLine(cavern[L_out[L_out.size()-2]].id_, cavern[L_out[L_out.size()-1]].id_, hole);
  for (int i = startShift; i < L_out.size(); i++){
    ...
    if (monoIndex.size() == 0){
      ...
      if ((G::isMonotoneLine(cavern[p_i1].id_, cavern[p_i2].id_, hole) == 0) || ((i == L_out.size()-1) && (monoIndex.size() >= 2))){
        ...
        translateFactor = r;
        trace() << "side " << side << "side0 " << side0;
        if (side != side0) translateFactor = (-r + kmax);
        trace() << "kmax " << kmax << " translateFactor " << translateFactor;
        auto translatedLine = G::translate(monoLine, translateFactor*side);
  ...
  // }
  trace() << "out ratio " << (calculatePathLength(result)/l_out);
  trace() << "shortest path length " << l_out;

  auto flattenResult = G::flatten(result);

  flattenResult.push_back(cavern[L_out[L_out.size()-1]]);
  auto result_pair = std::make_tuple(flattenResult, side*side0, translateFactor);
...
// vector<Point> MlpRoutingv2::findPathAroundHole(Point from, Point to, vector<Point> &hole, double r) {
...
vector<Point> MlpRoutingv2::findPathAroundHole(Point from, Point to, vector<Point> &hole, double r) {
  ...
  double theta = 0;
  double l_arround = 0;
  for (int i=0; i<L_arround.size()-1; i++){
  ...
  trace() << "l_arround " << l_arround << " theta " << theta;
  for (int i=1; i<L_arround.size()-1; i++){
    Point X_i = hole[L_arround[i]];
    Point X_i1 = hole[L_arround[i-1]];
    Point X_i2 = hole[L_arround[i+1]];    
    theta += M_PI - G::angle(&X_i, &X_i1, &X_i2);
  }

  //for (int i=0; i<5; i++) trace() << "OK5";

  double baseK = eps*l_arround/(theta+2);
  if (r == -1){
    int delta = getRandomNumber(0, RANGE);
    r = baseK / RANGE * delta;
  }
  ...
  trace() << "arround sign " << sign;
  vector<Point> L_arroundPath;
  for (int j=0; j<L_arround.size(); j++){
    L_arroundPath.push_back(hole[L_arround[j]]);
    debugPoint(hole[L_arround[j]], "black");
  }

  auto translatedPath = G::translate(L_arroundPath, r * sign);
  ...
  trace() << "arround ratio " << (calculatePathLength(result)/l_arround);
...
vector<Point> MlpRoutingv2::bypassHole(double sDist1, double sDist2, double dDist1, double dDist2, int s1, int s2, int d1, int d2, vector<Point> p) {
...
vector<int> MlpRoutingv2::bypassHole2(double sDist1, double sDist2, double dDist1, double dDist2, int s1, int s2, int d1, int d2, vector<Point> p) {
    Point ms = G::midpoint(p[s1], p[s2]);
    Point md = G::midpoint(p[d1], p[d2]);
...
void MlpRoutingv2::findViewLimitVertices(Point point, std::vector<Point> polygon, int &i1, int &i2) {
...
double MlpRoutingv2::calculatePathLength(vector<Point> p) {
    double length = 0;
...
double MlpRoutingv2::computeLevel(vector<Point> hole, int i, double max){
  int n = hole.size();
  Point p = hole[i];
