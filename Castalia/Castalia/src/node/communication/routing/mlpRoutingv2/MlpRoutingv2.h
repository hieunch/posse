...
#ifndef _MLPROUTINGV2_H_
#define _MLPROUTINGV2_H_
...
class MlpRoutingv2: public VirtualRouting {
  private:
    static int cnt;
    static int nextId;
    static double eps;
    ...
    bool receivedHole;
    // MlpRoutingv2-related member variables
    vector<Point> hole;
    vector<Point> holeConvexHull;
    vector<vector<CavernNode>> caverns;

    static map<tuple<tuple<Point, Point>, Point, Point, int>, tuple<vector<Point>, int, double> > outCavernCache;
  ...
  protected:
    ...
    tuple<vector<Point>, int, double> findPathOutCavern2(Point from, Point to, vector<Point> &hole, vector<CavernNode> &cavern, int delta);
